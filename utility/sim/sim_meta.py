...
class SimMeta():
    ...
    def check_dataset(self):
        super(SimMeta, self).check_dataset()
    ...
    def build_user_item_sim_CF(self, kfold, user_near_num=50, item_near_num=50, load_save_sim=False):
        ...
        if load_save_sim:
            self.user_sim = util.load_data('../data/sim/%s_08_uu_memf_cv0.pkl' % self.config.dataset_name)
        else:
            ...
            util.save_data(self.user_sim, '../data/sim/%s_08_uu_memf_cv0.pkl'  % self.config.dataset_name)

        # compute the k neighbors of user
        if load_save_sim:
            self.user_k_neibor = util.load_data(
                '../data/neibor/%s_08_uu_%s_neibor_memf_cv0.pkl'  % (self.config.dataset_name, user_near_num))
        ...
        # compute item-item similarity matrix
        print('构建 item-item meta 相似度矩阵  ...')
        if load_save_sim:
            self.item_sim = util.load_data('../data/sim/%s_08_ii_memf_cv0.pkl' % self.config.dataset_name)
        else:
            ...
            util.save_data(self.item_sim, '../data/sim/%s_08_ii_memf_cv0.pkl' % self.config.dataset_name)

        # compute the k neighbors of item
        if load_save_sim:
            self.item_k_neibor = util.load_data(
                '../data/neibor/%s_08_ii_%s_neibor_memf_cv0.pkl'  % (self.config.dataset_name, item_near_num))
