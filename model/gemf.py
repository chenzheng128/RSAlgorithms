...
from utility.sim.sim_ge import SimGe
from mts.metric import Metric
...
class GEMF(MF):
    ...
    def build_user_item_sim_CF(self, k):
        ...
        #     sim_model = SimGe
        #     print ("loading SimClass %s ..." % SimGe.__name__)
        # else:
        #     # TODO
    ...
    def train_model(self, k):
        super(GEMF, self).train_model(k) # 此时会 加载 init_mode()
...
if __name__ == '__main__':
    ...
    if first_fun:
        ...
        model = GEMF(user_near_num=usernum, item_near_num=itemnum)
    else:
        # 第二次运行，在运行上面语句 1 次后，即可加载缓存调试, 加载已经保存好的 sim 数据
        model = GEMF(user_near_num=usernum, item_near_num=itemnum, load_save_sim=True)
