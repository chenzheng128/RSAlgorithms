...
class CUCMMF(MF):
    """
    docstring for CUCMMF: CUC Multivariate Matrix Fraction
    ...
    def init_model(self, k):
        super(CUCMMF, self).init_model(k)
    ...
    def build_user_item_sim_CF(self, k):
        ...
        """
        from utility.sim.movielens import SimMovieLens
        from utility.sim.douban_movie import SimDoubanMovie
        sim_model = SimDoubanMovie()
        if self.config.dataset_name == "db":
            sim_model = SimDoubanMovie()
            print ("loading SimClass %s ..." % SimDoubanMovie.__name__)
        else:
            # TODO
    ...
    def train_model(self, k):
        super(CUCMMF, self).train_model(k) # 此时会 加载 init_mode()
...
if __name__ == '__main__':
    ...
    # model = CUCMMF(user_near_num=10, item_near_num=10)
    # 运行上面语句 1 次后，即可加速调试, 加载已经保存好的 sim 数据
    model = CUCMMF(user_near_num=50, item_near_num=50, load_save_sim=True)
    config = model.config
    ...
    print("### 参数影响性分析： 超参 调整 ")
    # config.lr = 0.002  # 学习率  
    # config.threshold = 100 # 判断收敛 
    # # config.lambdaP = 0.02
    # # config.lambdaQ = 0.002 # 正则化项目 
    # # model.item_near_num = 50 # item 近邻
    # # model.user_near_num = 50 # user 近邻
